...
function Result(props) {
	const result = {
		...
		qrImageUrl: encodeURI(
			`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://simple-share-sigma.vercel.app/download?id=${props.url}`
		),
	};
		...
		if (navigator.canShare) {
			...
			a.setAttribute(
				...
				'whatsapp://send?text=Hii there, I am sharing some files with you. Download them using the link \n' +
					`\n    at  https://simple-share-sigma.vercel.app/download?id=${props.url}`
			);
	...
	return (
		<div className='Result'>
			...
			<div className='textbox'>
				<input type='text' readOnly={true} id='copy' value={result.URL} />
				<button onClick={share} className='share' style={{ outline: 'none' }}>
					{' '}
					<img
			...
			<p style={{ width: '80%', textAlign: 'center', marginTop: '20px' }}>
				<u>Note</u>: Directly enter the code in the downloads section or scan
				the QR Code to Download Your Files
			</p>
			<button onClick={copy} className='CopyButton' style={{ outline: 'none' }}>
				Copy
			</button>
			<img
